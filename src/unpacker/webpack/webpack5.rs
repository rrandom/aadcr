            ...
            self.ctx
                ...
                .for_each(|(k, v)| {
                    ...
                    if k.as_str() == "default" {
                        ...
                        let export_default_kind =
                            ctx.ast.export_default_declaration_kind_expression(
                                v.clone_in(ctx.ast.allocator),
                            );

                        let st = ctx.ast.alloc_export_default_declaration(
                            Span::default(),
                            export_default_kind,
                            name,
                        );

                        let st = Statement::ExportDefaultDeclaration(st);
                        ...
                        if let Expression::Identifier(id) = v {
                            if id.name == k {
                                ...
                                let exported = ctx.ast.module_export_name_identifier_reference(
                                    Span::default(),
                                    k.clone_in(ctx.ast.allocator),
                                );

                                let s = ctx.ast.export_specifier(
                                    Span::default(),
                                    local,
                                    exported,
